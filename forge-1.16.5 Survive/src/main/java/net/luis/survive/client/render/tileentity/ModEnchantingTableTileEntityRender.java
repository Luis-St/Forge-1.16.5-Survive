package net.luis.survive.client.render.tileentity;
...
@OnlyIn(Dist.CLIENT)
public class ModEnchantingTableTileEntityRender extends TileEntityRenderer<ModEnchantingTableTileEntity> {
	...
	public ModEnchantingTableTileEntityRender(TileEntityRendererDispatcher p_i226010_1_) {
	...
	@Override
	public void render(ModEnchantingTableTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, 
			int combinedLightIn, int combinedOverlayIn) {
		
		matrixStackIn.push();
		matrixStackIn.translate(0.5D, 0.75D, 0.5D);
		float f = tileEntityIn.ticks + partialTicks;
		matrixStackIn.translate(0.0D, 0.1F + MathHelper.sin(f * 0.1F) * 0.01F, 0.0D);

		float f1;
		
		for (f1 = tileEntityIn.nextPageAngle - tileEntityIn.pageAngle; f1 >= Math.PI; f1 -=  Math.PI * 2F) {
