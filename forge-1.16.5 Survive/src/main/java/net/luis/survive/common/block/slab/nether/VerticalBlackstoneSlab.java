...
public class VerticalBlackstoneSlab extends AbstractVerticalSlab {
	
	public VerticalBlackstoneSlab() {

		super(AbstractBlock.Properties.from(Blocks.BLACKSTONE_SLAB));
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (hit.getFace().getOpposite() == state.get(VerticalBlackstoneSlab.FACING)
				...
				&& player.getHeldItem(handIn).getItem() == ModVerticalBlockItems.VERTICAL_BLACKSTONE_SLAB_ITEM.get()) {
