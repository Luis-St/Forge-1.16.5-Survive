...
public class ModEnchantingTableScreen extends ContainerScreen<ModEnchantingTableContainer> {
	...
	public ModEnchantingTableScreen(ModEnchantingTableContainer container, PlayerInventory playerInventory, ITextComponent textComponent) {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		for (int k = 0; k < 3; ++k) {
			...
			double d0 = mouseX - (i + 60);
			double d1 = mouseY - (j + 14 + 19 * k);
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int x, int y) {
		...
		for (int i1 = 0; i1 < 3; ++i1) {
			...
			if (l1 == 0) {
				...
				int j2 = 6839882;
				if (((l < i1 + 1 || this.minecraft.player.experienceLevel < l1)
						&& !this.minecraft.player.abilities.isCreativeMode) || this.container.enchantClue[i1] == -1) {
				...
				this.font.drawStringWithShadow(matrixStack, s, k1 + 86 - this.font.getStringWidth(s), j + 16 + 19 * i1 + 7, j2);
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		for (int j = 0; j < 3; ++j) {
			...
			if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, (double) mouseX, (double) mouseY) && k > 0) {
				...
				list.add((new TranslationTextComponent("container.enchant.clue",
						enchantment == null ? "" : enchantment.getDisplayName(l))).mergeStyle(TextFormatting.WHITE));
				...
				if (enchantment == null) {
					...
					if (this.minecraft.player.experienceLevel < k) {
						...
						list.add((new TranslationTextComponent("container.enchant.level.requirement",
								(this.container).enchantLevels[j])).mergeStyle(TextFormatting.RED));
